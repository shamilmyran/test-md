...
class Video extends Base {
	...
	_patch(data, msg) {
		...
		this.sudo = SUDO.split(",").includes(this.participant.split("@")[0]);
	...
	async warn(jid, message) {
		...
		readFile(path, (error, data) => {
			...
			if (totwarn >= 5) {
				...
				return message.client.sendMessage(message.jid, `_Total Warnings: ${totwarn}_`)
			}
		});
...
class ReplyMessage extends Base {
	...
	_patch(data, msg) {
		...
		this.sudo = SUDO.split(",").includes(data.participant.split("@")[0]);
		...
		if (data.quotedMessage && data.quotedMessage.imageMessage) {
			...
			this.sticker = {
				animated: data.quotedMessage.stickerMessage.isAnimated
			};
...
class Sticker extends Base {
	...
	_patch(data, msg) {
		...
		this.pushName = data.pushName;
		this.sudo = SUDO.split(",").includes(data.sender.split("@")[0]);
		this.timestamp =
			typeof data.messageTimestamp === "object" ?
			data.messageTimestamp.low :
			data.messageTimestamp;
